...
skynet.start(function()
	skynet.dispatch("lua", function (_,_,id)
		...
		print("code", code, "method", method, "url", url)
		if code then
			if code ~= 200 then
				response(id, interface.write, code)
			else
				local func = router[url]
				if not func then
					local tmp = {}
					if header.host then
					...
					table.insert(tmp, "404 FILE NOT FOUND\n")
					response(id, interface.write, code, table.concat(tmp,"\n"))
				else
					local ret = func()
					local tmp = {}
					if type(ret) == "table" then
						local cjsonText = cjson.encode(ret)
						table.insert(tmp, cjsonText)
					else
